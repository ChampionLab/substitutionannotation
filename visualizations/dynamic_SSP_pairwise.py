...
dfBaseBySample['logIntensity'] = np.log10(dfBaseBySample['Intensity'].replace(0,np.nan))
...
def render_content(tab):
    ...
    html.Div(
        children=[
            dcc.Graph(id='freq_scatter',style={'width':'40%'}),
            dcc.Graph(id='uniqueToA',style={'width':'29%'}),
            dcc.Graph(id='uniqueToB',style={'width':'29%'}),
        ],
...
@app.callback(
    ...
    Output('uniqueToA','figure'),
    Output('uniqueToB','figure'),
...
def update_frequency_scatter(freq_scatter_x,freq_scatter_y,signaltype):
   ...
   #Mask for substitutions missing an intensity in one or more samples
    ...
    #Plot of unique data vs. cognate in other sample

    #Parse data
    uniqueToX = dfplot.loc[missingInY,[xSignal,'Modified Peptide']]
    uniqueToY = dfplot.loc[missingInX,[ySignal,'Modified Peptide']]
    #Get base peptide from modified peptide
    uniqueToX['Peptide'] = uniqueToX['Modified Peptide'].str.replace('\[.*\]','',regex=True)
    uniqueToY['Peptide'] = uniqueToY['Modified Peptide'].str.replace('\[.*\]','',regex=True)
    #Merge base peptide intensity from other sample
    uniqueToX = uniqueToX.merge(dfBaseBySample.loc[dfBaseBySample['Sample']==freq_scatter_y,['Peptide','logIntensity']],how='left',on='Peptide')
    uniqueToY = uniqueToY.merge(dfBaseBySample.loc[dfBaseBySample['Sample']==freq_scatter_x,['Peptide','logIntensity']],how='left',on='Peptide')
    uniqueToX['logIntensity'] = uniqueToX['logIntensity'].replace(np.nan,0)
    uniqueToY['logIntensity'] = uniqueToY['logIntensity'].replace(np.nan,0)

    #Find unique substitutions that would presumably have an abundance above the detection limit in the other sample
    #And add a boolean vector for coloring in the plot
    uniqueToX['Expected Abundance'] = uniqueToX[xSignal] + uniqueToX['logIntensity']
    uniqueToY['Expected Abundance'] = uniqueToY[ySignal] + uniqueToY['logIntensity']
    uniqueToX['Colored'] = uniqueToX['Expected Abundance'] >= 4
    uniqueToY['Colored'] = uniqueToY['Expected Abundance'] >= 4
    #Add boolean vector where the cognate was not found in the other sample
    noCognateX = uniqueToY['logIntensity'] == 0
    noCognateY = uniqueToX['logIntensity'] == 0

    #Plot Unique to X
    pltA = px.scatter(uniqueToX.loc[~noCognateY,:],x='logIntensity',y=xSignal,color='Colored',
                      color_discrete_map={True:'orange',False:'grey'})
    pltA.update_traces(marker={'size': 3})

    # Y=X line
    pltA.add_trace(go.Scatter(x=[4,uniqueToX['logIntensity'].max()],y=[0,4-uniqueToX['logIntensity'].max()],
                             mode='lines',marker_color='rgba(130,130,130)',
                            showlegend=False,
                            line=dict(width=1,dash='dot'),
                            name='y=x'))
    # Violin of points missing cognate intensity
    pltA.add_trace(go.Violin(y=uniqueToX.loc[noCognateY,xSignal],
                            side='positive',
                            box_visible=False,
                            showlegend=False,
                            marker={'opacity':0},
                            line_color='grey', width = 4,
                            x0=0))
    pltA.update_layout(
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)")
    pltA.update_xaxes(showgrid=True, gridwidth=0.125, gridcolor='rgba(200,200,200,66)',
                     zerolinewidth = 0.125, zerolinecolor='rgba(200,200,200,66)',title_text=f'Cognate logIntensity in {freq_scatter_y}')
    pltA.update_yaxes(showgrid=True, gridwidth=0.125, gridcolor='rgba(200,200,200,66)',
                     zerolinewidth = 0.125, zerolinecolor='rgba(200,200,200,66)',title_text=f'{signaltype} of substitution unique to {freq_scatter_x}')
    pltA.layout.update(showlegend=False)
    
    #Plot Unique to Y
    pltB = px.scatter(uniqueToY.loc[~noCognateX,:],x='logIntensity',y=ySignal,color='Colored',
                      color_discrete_map={True:'mediumpurple',False:'grey'})
    pltB.update_traces(marker={'size': 3})
    # Y=X line
    pltB.add_trace(go.Scatter(x=[4,uniqueToY['logIntensity'].max()],y=[0,4-uniqueToY['logIntensity'].max()],
                             mode='lines',marker_color='rgba(130,130,130)',
                            showlegend=False,
                            line=dict(width=1,dash='dot'),
                            name='y=x'))
    # Violin of points missing cognate intensity
    pltB.add_trace(go.Violin(y=uniqueToY.loc[noCognateX,ySignal],
                            side='positive', width = 4,
                            box_visible=False,
                            showlegend=False,
                            marker={'opacity':0},
                            line_color='grey',
                            x0=0))
    pltB.update_layout(paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)")
    pltB.update_xaxes(showgrid=True, gridwidth=0.125, gridcolor='rgba(200,200,200,66)',
                     zerolinewidth = 0.125, zerolinecolor='rgba(200,200,200,66)',title_text=f'Cognate logIntensity in {freq_scatter_x}')
    pltB.update_yaxes(showgrid=True, gridwidth=0.125, gridcolor='rgba(200,200,200,66)',
                     zerolinewidth = 0.125, zerolinecolor='rgba(200,200,200,66)',title_text=f'{signaltype} of substitution unique to {freq_scatter_y}')
    pltB.layout.update(showlegend=False)

    #Write report of unique values
    dfout = pd.concat([uniqueToX,uniqueToY],ignore_index=True)
    dfout = dfout[dfout['Colored']]
    pathout = os.path.join(sA.outputdir,'dynamicVisualizations',f'Substitutions Unique in {freq_scatter_x} v {freq_scatter_y}.csv')
    if not os.path.exists(pathout):
        dfout.to_csv(pathout)

    return fig,pltA,pltB,textout
#%%
